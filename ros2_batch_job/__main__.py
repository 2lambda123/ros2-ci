...
if sys.platform in ("darwin"):
    pip_dependencies += ["cryptography", "lxml", "netifaces"]
...
def main(sysargv=None):
    ...
    if (sys.platform.lower().startswith("linux")
            and platform.linux_distribution()[2] == "xenial"):
        blacklisted_package_names += [
            "image_tools_py",
            "qt_dotgraph",
        ]
...
def process_coverage(args, job):
    ...
    for line in output.decode().splitlines():
        ...
        outfile = os.path.join(package_build_path,
                               package_name + ".coverage.xml")
...
def run(args, build_function, blacklisted_package_names=None):
    ...
    args.installspace = ("install space" if "installspace"
                         in args.white_space_in else "install")
    ...
    if args.do_venv:
        ...
        if args.os != "linux":
            ...
            job.run(
                [sys.executable, "-m", "pip", "install", "-U", "virtualenv"])
    ...
    with change_directory(args.workspace):
        ...
        if not args.src_mounted:
            ...
            if args.test_branch is not None:
                ...
                info(
                    "Attempting to merge all repositories to the '__ci_default' branch"
                )
                vcs_custom_cmd = vcs_cmd + [
                    "custom",
                    ".",
